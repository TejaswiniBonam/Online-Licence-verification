...
String cred = req.getParameter("vehicle");
...
if(rs.next()){
	...
	ResultSet rs2 = stm.executeQuery("select * from theft where vid='"+cred+"'");
	if(rs2.next()){
		out.println("<h1>THEFT CASE MATCH FOUND</h1><br>");
	}else{
		out.println("<h1>NO THEFT CASE MATCHED</h1><br>");
	}
